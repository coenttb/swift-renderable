...
struct PerformanceTests {
    ...
    func styleDeduplicationPerformance() throws {
        ...
        let result = clock.measure {
            ...
            print(
                "   Deduplication efficiency: \(cssRuleCount) rules for \(itemCount) styled elements"
            )
    ...
    func configurationPerformanceComparison() throws {
        ...
        let compactTimeInSeconds =
            Double(compactResult.components.seconds) + Double(compactResult.components.attoseconds)
            / 1e18
        let prettyTimeInSeconds =
            Double(prettyResult.components.seconds) + Double(prettyResult.components.attoseconds)
            / 1e18
        ...
        #expect(
            performanceDifference < 2.0,
            "Configuration performance should be similar (within 2x)"
        )
    }
