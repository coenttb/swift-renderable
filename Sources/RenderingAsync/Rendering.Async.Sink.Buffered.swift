...
extension Rendering.Async.Sink {
    ...
    ///
    /// This sink wraps `AsyncChannel` to provide bounded memory streaming.
    ...
    /// is slow, `Rendering.Async.Sink.Buffered` applies backpressure by suspending the
    /// producer until chunks are consumed. This ensures memory is bounded.
    ///
    /// ## Usage
    ///
    /// ```swift
    /// let sink = Rendering.Async.Sink.Buffered(chunkSize: 4096)
    ///
    /// // Producer task
    /// Task {
    ///     await sink.write("<html>".utf8)
    ///     await sink.write("<body>".utf8)
    ///     // ... more writes ...
    ///     await sink.finish()
    /// }
    ///
    ...
    /// }
    /// ```
    public actor Buffered: Rendering.Async.Sink.`Protocol` {
        private let channel: AsyncChannel<ArraySlice<UInt8>>
        private var buffer: [UInt8]
        private let chunkSize: Int

        /// Creates a new buffered sink with its own channel.
        ///
        ...
        /// Creates a new buffered sink using an external channel.
        ///
        /// - Parameters:
        ...
        /// Flush remaining bytes and finish the sink.
        ///
        /// Call this when rendering is complete to send any remaining buffered
        /// bytes and signal to consumers that the sink is finished.
